			       ...
			       unsigned param, unsigned argument)
{
	...
	switch (param) {
		...
		case PIN_CONFIG_BIAS_PULL_PIN_DEFAULT:
			if (argument) {
				int func = val & K210_PCF_MASK;
				int mode_id = k210_pcf_infos[func].mode_id;
				u32 def = k210_pc_mode_id_to_mode[mode_id];

				val &= ~K210_PC_BIAS_MASK;
				val |= def & K210_PC_BIAS_MASK;
			}
			break;
		...
		case PIN_CONFIG_OUTPUT:
			k210_pc_pinmux_set(dev, pin_selector,
					   K210_PCF_CONSTANT);
